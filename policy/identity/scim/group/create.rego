# METADATA
# description: Policy for provisioning a SCIM group (ADR 0024 §3, §8)
package identity.scim.group.create

import data.identity

# Create a new SCIM-provisioned group.
#
...
violation contains {"field": "domain_id", "msg": "provisioning a SCIM group in a domain different to the domain scope requires `admin` role."} if {
	not "admin" in input.credentials.roles
	not identity.domain_matches_domain_scope
}

violation contains {"field": "roles", "msg": "provisioning a SCIM group requires a scim_provisioner or manager role with the domain scope."} if {
	not "admin" in input.credentials.roles

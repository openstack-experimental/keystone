...
allow if { input.credentials.user_id == input.existing.application_credential.user_id }
...
violation contains {"field": "user_id", "msg": "deleting application credentials of a different user is not allowed."} if {
    ...
    input.credentials.user_id != input.existing.application_credential.user_id
}
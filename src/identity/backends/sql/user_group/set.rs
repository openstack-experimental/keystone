    ...
    IDP: AsRef<str>,
{
    ...
    if !groups_to_remove.is_empty() {
        println!("to remove {:?}", groups_to_remove);
...
mod tests {
    ...
    async fn test_expiring_add_and_remove() {
        ...
        set_user_groups_expiring(&db, "u1", vec!["g2", "g4", "g5", "g0"], "idp_id")
            .await
            .unwrap();
        ...
        assert_eq!(
            ...
            [
                ...
                Transaction::from_sql_and_values(
                    DatabaseBackend::Postgres,
                    r#"INSERT INTO "expiring_user_group_membership" ("user_id", "group_id", "idp_id") VALUES ($1, $2, $3), ($4, $5, $6) RETURNING "user_id", "group_id", "idp_id""#,
                    [
                        "u1".into(),
                        "g0".into(),
                        "idp_id".into(),
    ...
    async fn test_expiring_only_add() {
        ...
        set_user_groups_expiring(&db, "u1", vec!["g1", "g2", "g3", "g4", "g5"], "idp_id")
            .await
            .unwrap();
        ...
        assert_eq!(
            ...
            [
                ...
                Transaction::from_sql_and_values(
                    DatabaseBackend::Postgres,
                    r#"INSERT INTO "expiring_user_group_membership" ("user_id", "group_id", "idp_id") VALUES ($1, $2, $3) RETURNING "user_id", "group_id", "idp_id""#,
                    ["u1".into(), "g5".into(), "idp_id".into()]
                ),
            ]
        );
    }
    ...
    async fn test_expiring_only_delete() {
        ...
        set_user_groups_expiring(&db, "u1", vec!["g2", "g4"], "idp_id")
            .await
            .unwrap();
    ...
    async fn test_expiring_no_change() {
        ...
        set_user_groups_expiring(&db, "u1", vec!["g1", "g2", "g3", "g4"], "idp_id")
            .await
            .unwrap();

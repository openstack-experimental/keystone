...
pub async fn add_user_to_group_expiring<U: AsRef<str>, G: AsRef<str>, IDP: AsRef<str>>(
    ...
    let entry = expiring_user_group_membership::ActiveModel {
        user_id: Set(user_id.as_ref().into()),
        group_id: Set(group_id.as_ref().into()),
        idp_id: Set(idp_id.as_ref().into()),
        last_verified: NotSet,
    };
...
where
    ...
    IDP: AsRef<str>,
{
    //let last_verified = Utc::now().naive_utc();
...
mod tests {
    ...
    async fn test_add_expiring() {
        ...
        assert!(
            add_user_to_group_expiring(&db, "u1", "g1", "idp_id")
                .await
                .is_ok()
        );
        assert_eq!(
            ...
            [Transaction::from_sql_and_values(
                DatabaseBackend::Postgres,
                r#"INSERT INTO "expiring_user_group_membership" ("user_id", "group_id", "idp_id") VALUES ($1, $2, $3) RETURNING "user_id", "group_id", "idp_id", "last_verified""#,
    ...
    async fn test_bulk_expiring() {
        ...
        assert_eq!(
            ...
            [Transaction::from_sql_and_values(
                DatabaseBackend::Postgres,
                r#"INSERT INTO "expiring_user_group_membership" ("user_id", "group_id", "idp_id") VALUES ($1, $2, $3), ($4, $5, $6), ($7, $8, $9) RETURNING "user_id", "group_id", "idp_id""#,
                [
                    "u1".into(),
                    "g1".into(),
                    "idp_id".into(),

...
impl_deleter!(
    ...
    delete_k8s_auth_configuration
);

pub async fn create_k8s_auth_configuration(
    state: &ServiceState,
    data: K8sAuthConfigurationCreate,
) -> Result<AsyncResourceGuard<K8sAuthConfiguration, ServiceState>> {
    let res = state
        .provider
        .get_k8s_auth_provider()
        .create_k8s_auth_configuration(state, data)
        .await
        .unwrap();

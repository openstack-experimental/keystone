...
async fn test_set_user_groups_expiring() -> Result<(), Report> {
    ...
    state
        .provider
        .get_identity_provider()
        .set_user_groups_expiring(&state, &user.id, groups, "idp_id", Some(&now))
        .await?;
...
async fn test_remove_user_from_group_expiring() -> Result<(), Report> {
    ...
    state
        .provider
        .get_identity_provider()
        .add_user_to_group_expiring(&state, &user.id, &group.id, "idp_id")
        .await?;
    state
        .provider
        .get_identity_provider()
        .remove_user_from_group_expiring(&state, &user.id, &group.id, "idp_id")
        .await?;
...
async fn test_remove_user_from_groups_expiring() -> Result<(), Report> {
    ...
    state
        .provider
        .get_identity_provider()
        .remove_user_from_groups_expiring(&state, &user.id, to_remove, "idp_id")
        .await?;

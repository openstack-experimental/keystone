...
//! -- the closest in-repo equivalent to the ADR §6
//! `KeystoneNativeJwtMiddleware`, since that Python file itself ships in
//! downstream service repos, not here. Beyond the happy path, also proves a
//! token minted for one domain is rejected against another domain's JWKS/`aud`
//! (ADR §5, "Domain Key Isolation").

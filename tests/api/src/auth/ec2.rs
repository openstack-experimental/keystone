...
/// Order the parameters by raw name and join them percent-encoded.
...
/// ```text
/// StringToSign = VERB + "\n" + Host + "\n" + Path + "\n" + CanonicalQuery
/// CanonicalQuery = join("&", "<enc(k)>=<enc(v)>" ordered by raw k byte order)
/// Signature = base64(HMAC-SHA256(secret, StringToSign))
/// ```
///
/// The spec orders by parameter *name* only, so the value must never act as
/// a tiebreaker and the encoded form must never affect the ordering.
...
mod tests {
    ...
    #[test]
    fn canonical_query_orders_by_raw_key() {
        let params = HashMap::from([
            ("a/".to_string(), "1".to_string()),
            ("a-".to_string(), "2".to_string()),
        ]);
        // `-` (0x2D) sorts after `%` (0x25), so encoding before sorting would
        // swap these two parameters and sign a different string.
        assert_eq!(canonical_query(&params), "a-=2&a%2F=1");
    }
}

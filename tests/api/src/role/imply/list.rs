...
async fn test_list_implied_role() -> Result<()> {
    ...
    assert_eq!(group.prior_role.id, prior_role.id);
    assert_eq!(group.prior_role.name, prior_role.name);
    assert_eq!(group.implies.len(), 2);

    let implied_ids: Vec<_> = group.implies.iter().map(|r| r.id.as_str()).collect();
...
async fn test_list_implied_role_isolated() -> Result<()> {
    ...
    assert_eq!(group1.prior_role.id, prior1.id);
    assert_eq!(group1.implies.len(), 1);
    assert_eq!(group1.implies[0].id, implied1.id);

    assert_eq!(group2.prior_role.id, prior2.id);
    assert_eq!(group2.implies.len(), 0);

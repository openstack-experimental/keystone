...
#[tokio::test]
#[traced_test]
async fn test_create_mapping_ruleset() -> Result<()> {
    ...
    assert_eq!(ruleset.domain_id.as_deref(), Some("default"));
...
#[tokio::test]
#[traced_test]
async fn test_create_global_mapping_ruleset() -> Result<()> {
    let test_client = Arc::new(AsyncOpenStack::new(&CloudConfig::from_env()?).await?);
    ...
    assert!(ruleset.domain_id.is_none());
    assert!(ruleset.enabled);
    assert_eq!(ruleset.rules.len(), 1);
    assert_eq!(ruleset.rules[0].name, "test-rule");

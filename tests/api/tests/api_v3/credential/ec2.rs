...
//! |------------------------------------------------------|-----|-----|-----|
//! | POST   /            | `test_ec2_credential_create_success_owner`  | `test_ec2_credential_create_forbidden_cross_user` | `test_ec2_credential_create_unauthorized` |
//! | GET    /{access}    | `test_ec2_credential_show_success_owner`    | `test_ec2_credential_show_forbidden_cross_user`   | `test_ec2_credential_show_unauthorized`   |
//! | GET    /            | `test_ec2_credential_list_success_owner`    | `test_ec2_credential_list_forbidden_cross_user`   | `test_ec2_credential_list_unauthorized`   |
//! | DELETE /{access}    | `test_ec2_credential_delete_success_owner`  | `test_ec2_credential_delete_forbidden_cross_user` | `test_ec2_credential_delete_unauthorized` |
//!
...
//!
//! `Ec2Credential`'s derived `Debug` carries the plaintext secret, which the
//! assertion helpers print on an unexpected success, so asserted values are
//! reduced to their access key first.
...
async fn test_ec2_credential_create_forbidden_cross_user() -> Result<()> {
    ...
    assert_forbidden(
        create_ec2_credential(&a.session, &b.user.id, &b.project.id)
            .await
...
async fn test_ec2_credential_show_forbidden_cross_user() -> Result<()> {
    ...
    assert_forbidden(
        get_ec2_credential(&a.session, &b.user.id, &cred.access)
            .await
            .map(|cred| cred.access),
...
async fn test_ec2_credential_list_success_owner() -> Result<()> {
    ...
    let creds = list_ec2_credentials(
        &owner.session,
        &owner.user.id,
        Ec2CredentialListRequest::default(),
    )
    .await?;
    assert!(
...
async fn test_ec2_credential_delete_success_owner() -> Result<()> {
    ...
    assert_status(
        get_ec2_credential(&admin, &owner.user.id, &cred.access)
            .await
            .map(|cred| cred.access),

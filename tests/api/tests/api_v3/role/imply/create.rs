...
#[tokio::test]
#[traced_test]
async fn test_create_implied_role() -> Result<()> {
    ...
    let rule = create_implied_role(&tc, &prior_role.id, &implied_role.id).await?;

    assert_eq!(rule.prior_role.id, prior_role.id);
    assert_eq!(rule.implies.id, implied_role.id);
    assert_eq!(rule.prior_role.name, prior_role.name);
    assert_eq!(rule.implies.name, implied_role.name);

...
code), see the [OAuth2 / OIDC user guide](user.md).
...
legacy bootstrap). Normal domain creation via the API does this automatically;
...
through unchanged to the legacy Fernet filter chain, so rollout is incremental

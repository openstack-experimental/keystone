...
added for the Python Keystone identity provider and necessary protocols) A
federation rework assumes model change. To keep it working with the
python-keystone
...
the Python Keystone identity provider together with necessary idp protocols).
...
a volume into the container). The `keystone-manage token setup` and
`keystone-manage token rotate` commands manage the repository, and running
services reload key changes without a restart. See
[Fernet tokens](../admin/tokens/fernet.md) for configuration and operational procedures.

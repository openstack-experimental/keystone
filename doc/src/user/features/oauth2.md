...
provider rather than consuming it, see the [administrator guide](../../admin/features/oauth2.md).
...
- `openstack:api` — a distinct, explicit scope. Only when this is requested
  **and** granted does the returned `access_token` carry OpenStack authorization
  ...
  get a minimal identity token good only for calling Keystone's own `/userinfo`
  — not usable against Nova/Neutron/etc.

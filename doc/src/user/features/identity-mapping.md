# Identity Mapping Rules and API

Domain managers use the Identity Mapping Engine to resolve external identities
from federation, Kubernetes, SPIFFE, and API clients into Keystone principals
and security contexts. Instead of hardcoding authentication logic for each
provider, Keystone evaluates ordered rules that the domain manager controls.

Deployment configuration and cluster-wide operational requirements are covered
in the [administrator guide](../../admin/features/identity-mapping.md).
...
After the operator registers the SPIFFE trust domain, create a mapping ruleset
with source type `spiffe` and the appropriate `trust_domain`. Match rules
against `spiffe.id` or `spiffe.trust_domain` claims. Operator-only SPIFFE and
admin-interface configuration belongs in the
[administrator guide](../../admin/features/identity-mapping.md).
...
[Kubernetes TokenReview Authentication](kubernetes-auth.md).
...
| ---------- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Create** | `POST /v4/mappings/rulesets`                       | Create a new ruleset. Requires `source`, `domain_resolution_mode`, `enabled`, and `rules`.                                                                                   |
| **List**   | `GET /v4/mappings/rulesets`                        | List rulesets. Filterable by `domain_id`, `enabled`, `limit`, and `marker`.                                                                                                  |
| **Show**   | `GET /v4/mappings/rulesets/{mapping_id}`           | Get detailed rule definitions for a ruleset.                                                                                                                                 |
| **Update** | `PUT /v4/mappings/rulesets/{mapping_id}`           | Toggle `enabled` state, update `allowed_domains`, or replace the entire `rules` array. Immutable fields (`domain_id`, `source`, `domain_resolution_mode`) cannot be changed. |
| **Delete** | `DELETE /v4/mappings/rulesets/{mapping_id}`        | Remove a ruleset.                                                                                                                                                            |
| **Mutate** | `POST /v4/mappings/rulesets/{mapping_id}/rules/mutate` | Insert, update, or delete individual rules atomically.                                                                                                                    |
...
The operator must configure the cluster salt and enforce cluster-wide security
limits before mapping-backed authentication can be used. See

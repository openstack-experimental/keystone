...
use crate::webauthn::{
    ...
    authenticator_assertion_response_raw::AuthenticatorAssertionResponseRaw,
...
impl From<webauthn_rs_proto::auth::RequestChallengeResponse>
    for PasskeyAuthenticationStartResponse
{
    fn from(val: webauthn_rs_proto::auth::RequestChallengeResponse) -> Self {
        Self {
...
impl TryFrom<PasskeyAuthenticationFinishRequest> for webauthn_rs_proto::auth::PublicKeyCredential {
    type Error = WebauthnError;
    fn try_from(req: PasskeyAuthenticationFinishRequest) -> Result<Self, Self::Error> {
        Ok(webauthn_rs_proto::auth::PublicKeyCredential {
            id: req.id,

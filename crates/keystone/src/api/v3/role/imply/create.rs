...
#[utoipa::path(
    ...
    operation_id = "/roles/prior_role/implies/implied_role:put",
    params(
        ("prior_role_id" = String, Path, description = "The prior role ID."),
        ("implied_role_id" = String, Path, description = "The implied role ID.")
    ),
    responses(
        (status = CREATED, description = "Role imply rule created.", body = RoleImplyResponse),
...
pub(super) async fn create(
    ...
    state
        .policy_enforcer
        .enforce(
            "identity/role/imply_rule/create",
            &user_auth,
            json!({
                "role_imply_rule": {

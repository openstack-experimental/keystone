...
//! compiled module. All four host functions (ADR §6 A-D:
//! [`HostFunctions::http_fetch`], [`HostFunctions::provision_user`],
//! [`HostFunctions::find_user`], [`HostFunctions::assign_role`]) are
//! registered per plugin and each independently gated by that plugin's
//! configured `capabilities`. Dispatch of real
//! `authenticate`/`mapping`/`route` requests from an auth method lives in
//! `openstack-keystone-core` (`auth_plugin_auth`), which depends on this
...
impl WasmPluginRegistry {
    ...
    /// each one's `[auth_plugin.<name>]` entry in `configs`. Returns the

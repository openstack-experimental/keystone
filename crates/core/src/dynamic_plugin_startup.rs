...
/// `state.dynamic_plugin_registry`/`state.core_host_functions`. Never fails
...
pub async fn load_dynamic_plugins(
    ...
    let (section, configs) = {
        ...
        (cfg.dynamic_plugins.clone(), cfg.dynamic_plugin.clone())
    };
...
mod tests {
    use super::*;
    use crate::dynamic_plugin_http::FetchResponse;
    ...
    async fn test_load_dynamic_plugins_with_no_configured_plugins_leaves_registry_empty() {
        ...
        assert!(state.dynamic_plugin_registry.read().await.is_empty());
    }
}

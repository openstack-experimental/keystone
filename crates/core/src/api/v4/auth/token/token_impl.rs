...
use openstack_keystone_api_types::v4::auth::token::{
    System, Token as ApiToken, TokenBuilder, UserBuilder,
};
...
pub async fn build_api_token_v4(
    token: &ProviderToken,
    state: &ServiceState,
) -> Result<ApiToken, KeystoneApiError> {
...
mod tests {
    ...
    async fn test_from_unscoped() {
        ...
        let api_token = build_api_token_v4(
    ...
    async fn test_from_project_scoped() {
        ...
        let api_token = build_api_token_v4(&token, &state).await.unwrap();

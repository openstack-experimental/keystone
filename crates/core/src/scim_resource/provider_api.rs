...
//! # SCIM resource index provider API
...
/// SCIM resource ownership index provider interface (ADR 0024 §3.A).
///
/// Bookkeeping-only: this provider never itself emits CADF audit events —
/// the SCIM resource handlers (Users/Groups) that call it are responsible
/// for auditing the actual Identity mutation they perform alongside the
/// index write.
#[async_trait]
pub trait ScimResourceApi: Send + Sync {
    ...
    /// (ADR 0024 §3.C step 1: the Ownership Fencing Algorithm).
    ...
    /// Fetch the ownership anchor by its realm-scoped `externalId`.

...
/// v1: `HMAC-SHA1(secret, concat(key+value for sorted(params,
/// key=str.lower)))`, Base64.
...
///    `req.signature`.
/// 3. If that fails and `req.host` carries a port, strip it and retry once (a
///    fresh HMAC instance is used — HMAC state must not be reused across
///    attempts).
...
mod tests {
    ...
    //   base64.b64encode(hmac.new(b"secret",
    // b"DescribeInstances2026-06-11T12:00:00Z", hashlib.sha1).digest())
    ...
    //   canonical = "&".join(quote(k) + "=" + quote(v, safe="-_~") for k, v in
    // sorted(params.items()))   msg = "GET\nidentity.example.com\n/\n" +
    // canonical   base64.b64encode(hmac.new(b"secret", msg.encode(),

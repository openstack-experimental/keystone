...
pub async fn authenticate_via_wasm_mapping_plugin(
    ...
    let response =
        match openstack_keystone_dynamic_plugin_runtime::decode_and_validate_mapping_response(
            &raw_response,
        ) {
            Ok(response) => response,
            Err(e) => {
...
mod mapping_acceptance_tests {
    ...
    /// forbidden per PR 0.1), so the identity/dynamic-plugin-identity mocks

...
//! raft index doesn't (see `crates/core/src/auth_plugin_identity/mod.rs`):
//! a auth-plugin-provisioned user is an ordinary Keystone user reachable
...
/// Hook that purges auth-plugin identity index entries for a user that
    ...
    async fn on_event(&self, event: &Event) {
        if let (Operation::Delete, EventPayload::User { id }) = (&event.operation, &event.payload) {
            ...
            if let Err(e) = self
                .state
                .provider
                .get_auth_plugin_identity_provider()

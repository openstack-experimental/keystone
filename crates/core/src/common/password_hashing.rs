...
mod tests {
    ...
    // --- Keystone Python cross-compatibility tests (Direction 2) ---
    //
    // Every hash string below was generated by the real, unmodified Keystone
    // Python hashers (keystone/common/password_hashers/{bcrypt,pbkdf2,scrypt}.py
    // — NOT Passlib) running against bcrypt 5.0 and cryptography 49.0, with the
    // same password_hashing.py pre-dispatch truncation policy this module
    // implements (`verify_length_and_trunc_password`) applied first. Each vector
    // was self-verified by the producing Python hasher before being pasted here.
    // These prove the Rust verifier accepts genuine Python-produced hashes.
    //
    // The reverse direction (Python verifying Rust-produced hashes) is covered
    // by the env-gated test_cross_verify_* tests below, which shell out to
    // tools/cross_verify.py. To regenerate this set, run tools/generate_kats.py
    // from a Keystone checkout with bcrypt + cryptography installed.

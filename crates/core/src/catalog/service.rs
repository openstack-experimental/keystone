    ...
    async fn add_endpoint_to_project<'a>(
        ...
        if let Some(vsc) = exec.ctx() {
            ...
            crate::audited_op! {
                ...
                operation: async {
                    backend_driver.add_endpoint_to_project(exec.state(), project_id, endpoint_id).await
                },
            ...
            crate::audited_op! {
                ...
                event: Event::new(
                    ...
                    EventPayload::ProjectEndpointGroup {
                        project_id: project_id.to_string(),
                        endpoint_group_id: endpoint_group_id.to_string(),
                    },
                ),
                operation: async {
                    backend_driver.add_endpoint_group_to_project(exec.state(), project_id, endpoint_group_id).await
                },
                on_audit_error: |_: AuditDispatchError| CatalogProviderError::Driver("audit dispatch failed".into()),
            }?
        } else {
    ...
    async fn remove_endpoint_from_project<'a>(
        ...
        if let Some(vsc) = exec.ctx() {
            ...
            crate::audited_op! {
                ...
                operation: async {
                    backend_driver.remove_endpoint_from_project(exec.state(), project_id, endpoint_id).await
                },
                on_audit_error: |_: AuditDispatchError| CatalogProviderError::Driver("audit dispatch failed".into()),
            }?
        } else {
    ...
    async fn remove_endpoint_group_from_project<'a>(
        ...
        if let Some(vsc) = exec.ctx() {
            ...
            crate::audited_op! {
                ...
                operation: async {
                    backend_driver.remove_endpoint_group_from_project(exec.state(), project_id, endpoint_group_id).await
                },
                on_audit_error: |_: AuditDispatchError| CatalogProviderError::Driver("audit dispatch failed".into()),
            }?
        } else {

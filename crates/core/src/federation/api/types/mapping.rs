...
impl From<api_mapping::MappingCreateRequest> for types::Mapping {
    fn from(value: api_mapping::MappingCreateRequest) -> Self {
        Self {
...
impl From<api_mapping::MappingUpdateRequest> for types::MappingUpdate {
    fn from(value: api_mapping::MappingUpdateRequest) -> Self {
        Self {
            name: value.mapping.name,
            idp_id: value.mapping.idp_id,
...
impl IntoResponse for types::Mapping {
    fn into_response(self) -> Response {
        (
            StatusCode::OK,
            Json(api_mapping::MappingResponse {
                mapping: api_mapping::Mapping::from(self),
            }),
        )
            .into_response()
    }
...
impl From<types::MappingType> for api_mapping::MappingType {
    fn from(value: types::MappingType) -> Self {
        match value {
            types::MappingType::Oidc => Self::Oidc,
            types::MappingType::Jwt => Self::Jwt,
        }
    }
}

impl From<api_mapping::MappingType> for types::MappingType {
    fn from(value: api_mapping::MappingType) -> Self {
        match value {
            api_mapping::MappingType::Oidc => Self::Oidc,
            api_mapping::MappingType::Jwt => Self::Jwt,
        }
    }
}

impl TryFrom<api_mapping::MappingListParameters> for types::MappingListParameters {
    type Error = KeystoneApiError;

    fn try_from(value: api_mapping::MappingListParameters) -> Result<Self, Self::Error> {
        Ok(Self {
            domain_id: value.domain_id,
            idp_id: value.idp_id,
            limit: value.limit,

...
//!
//! Parsing and validation of the `[auth_plugins]` / `[auth_plugin.*]`
...
use crate::common::{ProxyHeader, csv, csv_ipnet};
...
/// `[auth_plugins]` section: the list of configured plugin names, each of
/// which must have a corresponding `[auth_plugin.<name>]` section.
#[derive(Debug, Default, Deserialize, Clone)]
pub struct DynamicPluginsSection {
    /// Names of the dynamic plugins to load at startup. Each entry must
    /// have a matching `[auth_plugin.<name>]` section.
    #[serde(default, deserialize_with = "csv")]
    pub plugins: Vec<String>,

    /// CIDR blocks of reverse proxies trusted to append entries to the
    /// configured forwarding header for `full_auth`-mode dispatch's
    ...
    /// Defaults to the pre-existing `x_forwarded_for` behavior. Opt in to
    /// `forwarded` only when every trusted proxy owns that header.
    #[serde(default)]
...
/// A `[auth_plugins]`/`[auth_plugin.*]` configuration that failed one
/// of ADR 0025's fail-loud, config-load-time invariants.
#[derive(Debug, Error, PartialEq, Eq)]
pub enum DynamicPluginConfigError {
    #[error(
        "dynamic plugin `{0}` is listed in [auth_plugins] plugins but has no \
         [auth_plugin.{0}] section"
    )]
...
mod tests {
    ...
    fn parse(ini: &str) -> (DynamicPluginsSection, HashMap<String, DynamicPluginConfig>) {
        ...
        (wrapper.auth_plugins, wrapper.auth_plugin)
    }
...
"#;
    ...
    fn test_reserved_name_is_rejected() {
        ...
        // Sanity: replacement must have hit both the [auth_plugins] list

...
mod tests {
    ...
    use super::*;
    use crate::role::tests::get_k8s_auth_role_mock;
    ...
    async fn test_update() {
        ...
        assert_eq!(
            ...
            [Transaction::many(vec![
                ...
                Statement::from_sql_and_values(
                    DatabaseBackend::Postgres,
                    r#"UPDATE "kubernetes_auth_role" SET "bound_audience" = $1, "bound_service_account_names" = $2, "bound_service_account_namespaces" = $3, "enabled" = $4, "name" = $5, "token_restriction_id" = $6 WHERE "kubernetes_auth_role"."id" = $7 RETURNING "auth_instance_id", "bound_audience", "bound_service_account_names", "bound_service_account_namespaces", "domain_id", "id", "enabled", "name", "token_restriction_id""#,
                    [
                        "new_aud".into(),
                        "c".into(),
                        "nc".into(),

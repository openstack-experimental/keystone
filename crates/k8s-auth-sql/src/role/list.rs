...
fn get_list_query(
    ...
    if let Some(val) = &params.auth_instance_id {
        select = select.filter(kubernetes_auth_role::Column::AuthInstanceId.eq(val));
    }
...
mod tests {
    ...
    async fn test_query_configuration_id() {
        ...
        .to_string(PostgresQueryBuilder);
        assert!(query.contains("\"kubernetes_auth_role\".\"auth_instance_id\" = 'cid'"));
    }

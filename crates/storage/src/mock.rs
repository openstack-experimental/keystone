            ...
            match mutation {
                Mutation::Remove {
                    ...
                    let violation = violation.or_else(|| {
                        if expected_revision.is_some() && !metadata_map.contains_key(&key_str) {
                            Some(Violation {
                                r#type: "CONFLICT".to_string(),
                                subject: key_str.clone(),
                                description: format!(
                                    "removal CAS conflict: key {} does not exist (expected revision {})",
                                    key_str,
                    ...
                    if violation.is_none() {
                        ...
                        violations.push(violation.unwrap());
                    }
                }

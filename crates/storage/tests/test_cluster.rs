...
async fn test_purge_then_join_learner_catches_up_via_snapshot_inner() -> Result<()> {
    ...
    let upto = instance1
        .storage
        .last_log_index()
        .expect("node 1 must have a non-empty log after writes");
    ...
    let mut caught_up = false;
    for i in 0..100 {
        let idx = instance2.storage.last_log_index();
        println!("=== poll {i}: node 2 last_log_index={:?}", idx);
        if idx >= Some(upto) {
            caught_up = true;
            break;
        }

...
impl RaftBackend {
    /// Primary storage key:
    /// `auth_plugin_identity:v1:<plugin_name>:<external_id>`
    /// -> `user_id`.
    fn primary_key<P: AsRef<str>, E: AsRef<str>>(plugin_name: P, external_id: E) -> String {
        format!(
            "auth_plugin_identity:v1:{}:{}",
            plugin_name.as_ref(),
            external_id.as_ref()
        )
    }

    /// Reverse lookup key: `auth_plugin_identity:by_user:v1:<user_id>:
    ...
    fn reverse_key<U: AsRef<str>, P: AsRef<str>, E: AsRef<str>>(
        ...
        format!(
            "auth_plugin_identity:by_user:v1:{}:{}:{}",
            user_id.as_ref(),
            plugin_name.as_ref(),
            external_id.as_ref()
        )
    ...
    fn reverse_prefix<U: AsRef<str>>(user_id: U) -> String {
        format!("auth_plugin_identity:by_user:v1:{}:", user_id.as_ref())
    }

    /// Prefix covering every primary mapping for `plugin_name`. Disjoint from
    /// the `by_user:` reverse index (which diverges right after the shared
    /// `auth_plugin_identity:` token), and unambiguous because a plugin
    /// name - a builtin-checked auth-method name - can never contain a `:`.
    fn primary_prefix<P: AsRef<str>>(plugin_name: P) -> String {
        format!("auth_plugin_identity:v1:{}:", plugin_name.as_ref())
    }
    ...
    async fn list_by_plugin_impl(
        &self,
        storage: &dyn StorageApi,
        plugin_name: &str,
    ) -> Result<Vec<(String, String)>, StoreError> {
        let prefix = Self::primary_prefix(plugin_name);
        let entries = storage.prefix(prefix.as_bytes(), None).await?;
        let mut out = Vec::with_capacity(entries.len());
        for (key, envelope) in entries {
            // `external_id` is the key suffix after the plugin-scoped prefix;
            // it may itself contain `:`, so recover it by stripping the prefix
            // rather than splitting on `:`. The value is the `user_id`.
            let Some(external_id) = key.strip_prefix(prefix.as_str()) else {
                continue;
            };
            let user_id = envelope.try_deserialize::<String>()?.data;
            out.push((external_id.to_string(), user_id));
        }
        Ok(out)
    }
...
impl DynamicPluginIdentityBackend for RaftBackend {
    async fn create_or_resolve<'a>(
        ...
        let raft = state
            .storage
            .as_deref()
            .ok_or(AuthPluginIdentityProviderError::RaftNotAvailable)?;
    ...
    async fn list_by_plugin<'a>(
        ...
        let raft = state
            ...
            .as_deref()
            .ok_or(AuthPluginIdentityProviderError::RaftNotAvailable)?;
        self.list_by_plugin_impl(raft, plugin_name)
            .await
            .map_err(AuthPluginIdentityProviderError::raft)
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_list_by_plugin_returns_only_that_plugins_mappings() {
        let backend = RaftBackend::default();
        let storage = MockStorage::default();
        ...
        backend
            ...
            .await
            .unwrap();
        // `external_id` containing a `:` must round-trip intact.
        backend
            .create_or_resolve_impl(&storage, "acme", "ext:2", "u2")
            .await
            .unwrap();
        // A different plugin must not leak into acme's listing.
        backend
            .create_or_resolve_impl(&storage, "beta", "ext-3", "u3")
            .await
            .unwrap();

        let mut links = backend.list_by_plugin_impl(&storage, "acme").await.unwrap();
        links.sort();
        assert_eq!(
            links,
            vec![
    ...
    #[tokio::test]
    async fn test_list_by_plugin_with_no_mappings_is_empty() {
        let backend = RaftBackend::default();
        let storage = MockStorage::default();

        let links = backend
            .list_by_plugin_impl(&storage, "nonexistent")
            .await
            .unwrap();
